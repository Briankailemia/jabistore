...
export default function AdminTabs({ activeTab, setActiveTab, tabs: customTabs }) {
  const tabs = customTabs || defaultTabs

  return (
    <div className="bg-white rounded-lg shadow-sm border mb-8">
      <div className="border-b border-gray-200">
        <nav role="tablist" aria-label="Admin dashboard sections" className="flex space-x-8 px-6">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              role="tab"
